...
const ContractTab = ({ contracts, contractsIsLoading, contractsIsError }) => {
  ...
  const goToCreateEventPage = () => {
    navigate("addition", {
      state: { contactId: selectContract?.customerContactId },
    });
  };
    ...
    <Fragment>
      ...
      <div className="flex flex-row w-full py-2">
        <div className="flex flex-row w-full justify-between items-center mb-2 ">
          ...
          <Button type="primary" size="large" onClick={goToCreateEventPage}>
            Tạo sự kiện
          </Button>
          {/* {selectContract &&
            selectContract?.files?.[0]?.status === "ACCEPTED" && (
            )} */}
        </div>
      </div>
      ...
      {isContract ? (
        ...
        <>
          <Spin spinning={contractsIsLoading}>
            <div className="flex flex-wrap w-full justify-between">
              {/* card */}
              {!!contracts?.length ? (
                contracts?.map((contract) => (
                  <div
                    className=" flex w-[32%] bg-white h-fit mb-7 rounded-xl"
                    key={contract?.id}
                  >
